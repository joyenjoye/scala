...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Nothing = throw java.util.NoSuchElementException("Empty TweetSet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    def compare(tw1: Tweet, tw2: Tweet): Tweet ={
    ...
    else compare(left.mostRetweeted,right.mostRetweeted)
  }

  def descendingByRetweet: TweetList = Cons(this.mostRetweeted, left.union(right).descendingByRetweet)
